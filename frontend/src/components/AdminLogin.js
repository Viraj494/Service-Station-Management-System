...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:8070/admin/login', { email, password }).then((res) => {
  ...
  return (
    ...
    <form className="Login" onSubmit={handleSubmit}>
      <h2> Admin Login</h2><br></br><br></br>
      <label>Email</label><br></br>
      <input type="email" id="email" className="form-control" onChange={change} value={email} required/><br></br>

      <label>Password</label><br></br>
      <input type="password" id="password" className="form-control" onChange={change} value={password} required/><br></br><br></br>
